                ...
                <div className="h-[calc(100vh-40px)] pb-10 ">
                    <div className="h-[calc(100%-128px)]  overflow-auto px-2">
                        ...
                        {(Array.isArray(cart?.products) ? cart.products.filter(Boolean) : []).map(product => (
                            <div key={product?.variantId || Math.random()} className="flex justify-between items-center gap-5 mb-4 border-b pb-4">
                                <div className="flex gap-5 items-center">
                                    <Image src={product?.media || imgPlaceholder.src} height={100} width={100} alt={product?.name || 'Product'} className="w-20 h-20 rounded border" />

                                    <div >
                                        <h4 className="text-lg mb-1">{product?.name || 'Unnamed item'}</h4>
