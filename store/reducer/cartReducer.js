...
export const cartReducer = createSlice({
    ...
    reducers: {
        addIntoCart: (state, action) => {
            // Ensure product list has no null/undefined holes
            state.products = Array.isArray(state.products) ? state.products.filter(Boolean) : []
        ...
        removeFromCart: (state, action) => {
            const { productId, variantId } = action.payload
            state.products = (Array.isArray(state.products) ? state.products : []).filter((product) => !(product?.productId === productId && product?.variantId === variantId))
