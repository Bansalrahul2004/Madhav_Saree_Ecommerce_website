...
const CartPage = () => {
    ...
    useEffect(() => {
        ...
        const totalAmount = cartProducts.reduce((sum, product) => sum + (((product?.sellingPrice) || 0) * ((product?.qty) || 0)), 0)
                ...
                <div className='flex lg:flex-nowrap flex-wrap gap-10 my-20 lg:px-32 px-4'>
                    <div className='lg:w-[70%] w-full'>
                        <table className='w-full border'>
                            ...
                            <tbody>
                                {(Array.isArray(cart?.products) ? cart.products.filter(Boolean) : []).map(product => (
                                    <tr key={product?.variantId || Math.random()} className='md:table-row block border-b'>
                                        <td className='p-3'>
                                            <div className='flex items-center gap-5'>
                                                <Image src={product?.media || imgPlaceholder.src} width={60} height={60} alt={product?.name || 'Product'} />
                                                <div>
                                        ...
                                        <td className='md:table-cell flex justify-between md:p-3 px-3 pb-2'>
                                            ...
                                            <div className='flex justify-center'>
                                                <div className="flex justify-center items-center md:h-10 h-7 border w-fit rounded-full">
                                                    ...
                                                    <input type="text" value={product?.qty || 0} className="md:w-14 w-8  text-center border-none outline-offset-0" readOnly />
                                        ...
                                        <td className='md:table-cell flex justify-between md:p-3 px-3 pb-2 text-center'>
                                            ...
                                            <span>
                                                {(((product?.sellingPrice || 0) * (product?.qty || 0))).toLocaleString('en-IN', { style: 'currency', currency: 'INR' })}
                                            </span>
                                        </td>
