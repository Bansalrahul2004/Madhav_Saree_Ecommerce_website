...
const ProductDetails = ({ product, variant, colors, sizes, reviewCount }) => {
    ...
    useEffect(() => {
        if (cartStore?.count > 0 && Array.isArray(cartStore?.products)) {
            const safeProducts = cartStore.products.filter(Boolean)
            const existingProduct = safeProducts.findIndex((cartProduct) => (
